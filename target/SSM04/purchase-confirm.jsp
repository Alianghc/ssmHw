...
<head>
    ...
    <script language="JavaScript">
        $(document).ready(function () {
            if (location.href.indexOf("#") === -1) {
                ...
                $.ajax({
                    url: "http://localhost:8080/purchase/findAll",
                    type: "POST",
                    async:false
                });
                location.reload();
    ...
    <body>
                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="m-b-30">
                                    <button id="addToTable" class="btn btn-primary waves-effect waves-light" onclick="window.location.href='caigoulist-add.jsp'">Add <i class="fa fa-plus"></i></button>
                                </div>
                            </div>
                        </div>
                        <table class="table table-bordered table-striped" id="datatable-editable">
                           ...
                           <form >
                               <c:forEach items="${sessionScope.daizhixingPurchase}" var="adpPorder">
                               <tr>
                                        <td>${adpPorder.id}</td>
                                        <td>${adpPorder.date}</td>
                                        <td>${adpPorder.address}</td>
                                        <td>${adpPorder.state}</td>
                                        <td>${adpPorder.goods_id}</td>
                                        <td>
                                            <button type="submit" class="btn btn-success m-l-10"><a href="purchase-inputID.jsp?id=${adpPorder.id}">提交</a></button>
                                        </td>
                                    </tr>
                               </c:forEach>
                                    <c:forEach items="${sessionScope.yiquerenPurchase}" var="yadpPrder">
                                        <tr>
                                            <td>${yadpPrder.id}</td>
                                            <td>${yadpPrder.date}</td>
                                            <td>${yadpPrder.address}</td>
                                            <td>${yadpPrder.state}</td>
                                            <td>${yadpPrder.goods_id}</td>
        ...
        <script src="assets/jquery-datatables-editable/purchaseorderconfirmdatatables.editable.init.js"></script>
